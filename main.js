...
function getHP(playerDefence, playerHit, defence, hit) {
  if (hit.hit !== defence.defence) {
    playerDefence.changeHP(hit.value);
    playerDefence.renderHP();
    generateLogs('hit', playerHit, playerDefence, hit.value);
  } else generateLogs('defence', playerHit, playerDefence);
}
...
function generateLogs(type, player1, player2, damage) {
  ...
  switch (type) {
    case 'start':
      ...
      break;
    case 'hit':
      text = `${getTime()} - ${logs[type][randomDamage(logs[type].length - 1)]
    ...
    case 'defence':
      ...
      break;
    case 'end':
      text = logs.end[randomDamage(3 - 1)]
...
function getTextBattle(type, hitPlayer, defencePlayer, damage) {
  return `${getTime()} - ${logs[type][randomDamage(logs[type].length - 1)]
    .replace('[playerKick]', hitPlayer.name)
    .replace('[playerDefence]', defencePlayer.name)}. -${damage} [${
    defencePlayer.hp
  }/100]`;
}
...
formFight.addEventListener('submit', function (e) {
  ...
  getHP(player1, player2, player, enemy);
  getHP(player2, player1, enemy, player);
  showResult();
});
